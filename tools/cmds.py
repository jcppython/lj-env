...
script = sys.path[0] + "/execline.sh"
sshHosts = sys.path[0] + "/texts"
...
class SSHGO:
    ...
    def _parse_tree_from_config_file(self, config_file):
        ...
        for line in open(config_file, 'r'):
            ...
            #  node_host = node_infos[0] if len(node_infos) == 3 else line_con
            #  node_user = node_infos[1] if len(node_infos) == 3 else ''
            #  node_pass = node_infos[2] if len(node_infos) == 3 else ''

            new_node = {'level':tree_level,'expanded':expand,'line_number':line_number,'line':line,'sub_lines':[]}
            #  new_node = {'level':tree_level,'expanded':expand,'line_number':line_number,'line':node_host,'user':node_user,'password':node_pass,'sub_lines':[]}
